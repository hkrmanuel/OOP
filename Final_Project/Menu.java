...
public class Menu{
    ...
    // File objects for each restaurant's menu
    File Akornor = new File("AkornorMenu.txt");
    File Munchies = new File("MunchiesMenu.txt");
    File BigBen = new File("BigBenMenu.txt");

    // Arrays to store food names and prices
    public static String[] foodnames = new String[50];
    public static int[] prices = new int[50];
    ...
    /**
     * Displays the menu for Munchies restaurant.
     */
    public void displayMunchiesMenu(){
        try {
                Scanner filescan = new Scanner(Munchies);
                System.out.println("Munchies MENU\n" + "\tCODE\t\tFOOD ITEM\t\t\t\tPRICE");
                System.out.println("-------------------------------------------------------------------------------------------------------");

                // Reading and displaying menu items from file
                int i = 0;
                while (filescan.hasNext()) {

                    String foodname = filescan.next();
                    String price = filescan.next();
                    foodnames[i]= foodname;
                    prices[i] = Integer.parseInt(price);
                    i++;
                    System.out.printf("\t%d\t\t%-30s\tGH₵%s\n", i, foodname, price);
    ...
    // Method to display the Big Ben menu
    public void displayBigBenMenu(){
        try {
                // Opening a scanner to read from the BigBen file
                Scanner filescan = new Scanner(BigBen);
                ...
                int i = 0;
                while (filescan.hasNext()) {
                    // Reading food name and price from file
                    String foodname = filescan.next();
                    String price = filescan.next();
                    // Storing food name and price in arrays (assuming they are declared elsewhere)
                    foodnames[i]= foodname;
                    prices[i] = Integer.parseInt(price);
                    i++;
                    // Printing menu item details
                    System.out.printf("\t%d\t\t%-30s\tGH₵%s\n", i, foodname, price);
                }
        ...
        /* 
        // Sample menu items (commented out)
        System.out.println("  Breakfast");
